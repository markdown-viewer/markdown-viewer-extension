...
从 Excel/WPS 导出的表格常把合并单元格的空白填成与上一行相同的值，这种表格也应识别为树形并合并。
...
**预期效果：**
- 与"空白填充"版本完全一致："研发部"合并 3 行、"产品部"合并 2 行
- "前端组"合并 2 行、"后端组"1 行、"产品组"合并 2 行
- 成员、工资每行不同，不合并
...
**预期效果：**
- "研发部"合并 2 行 + 2 行（空白后的重复值开启新一段，与空白风格行为一致）
- "前端组"合并 2 行、"后端组"合并 2 行
- 备注列空白不合并
